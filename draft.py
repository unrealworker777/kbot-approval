...
- Если это комментарий к посту — 1-3 предложения, уместные под чужим постом.
  ГЛАВНОЕ: пиши только если есть что сказать ПО СУЩЕСТВУ — конкретика, правило из
  практики, спорный тезис. Если сказать можно только общее («формат интересный»,
  «важная тема», «это всегда даёт больше») — такую воду НЕ пиши вообще.
...
def generate_variants(kind, context_text, n=2, avoid=None):
    """Возвращает список из n РАЗНЫХ альтернативных черновиков."""
    avoid = list(avoid or [])
    variants = []
    for _ in range(n):
        seen = avoid + variants
...
FILTER_PROMPT = """Ты фильтр для эксперта по девелопменту (КРТ, ИЖС, земля, финмодели).
Решаешь, есть ли смысл ему писать комментарий под этим текстом.

ОТВЕЧАТЬ стоит, если текст про:
{relevant}

НЕ ОТВЕЧАТЬ, если это:
{stop}

Главный критерий: есть ли в тексте ПРЕДМЕТ для экспертной реплики по существу.
Если сказать можно только общие слова («формат интересный», «важная тема») — это НЕТ.
Если есть конкретика, кейс, цифры, норма, спорный тезис, ошибка — это ДА.

Ответь ровно одним словом: ДА или НЕТ."""


def is_relevant(text):
    """True, если по тексту есть смысл писать экспертный комментарий."""
    if config.TOPIC_FILTER != "on":
        return True
    system = FILTER_PROMPT.format(relevant=config.RELEVANT_TOPICS, stop=config.STOP_TOPICS)
    try:
        ...
        answer = "".join(b.text for b in r.content if b.type == "text").strip().upper()
        return answer.startswith("ДА")
    except Exception as e:
        print(f"Фильтр темы не сработал ({e}) — пропускаю дальше")
        return True   # при сбое лучше показать черновик, чем молча потерять
